...
if ($env:AZURE_ADLS_GEN2_STORAGE_ACCOUNT) {
  ...
  if ($env:AZURE_ADLS_GEN2_FILESYSTEM_PATH) { $adlsGen2FilesystemPathArg = "--datalakefilesystempath $env:AZURE_ADLS_GEN2_FILESYSTEM_PATH" }
  $adlsGen2FilesystemArg = ""
  if ($env:AZURE_ADLS_GEN2_FILESYSTEM) { $adlsGen2FilesystemArg = "--datalakefilesystem $env:AZURE_ADLS_GEN2_FILESYSTEM" }
}
if ($env:AZURE_USE_AUTHENTICATION) { $aclArg = "--useacls" }

# Optional Search Analyzer name if using a custom analyzer
if ($env:AZURE_SEARCH_ANALYZER_NAME) { $searchAnalyzerNameArg = "--searchanalyzername $env:AZURE_SEARCH_ANALYZER_NAME" }
if ($env:AZURE_VISION_ENDPOINT) { $visionEndpointArg = "--visionendpoint $env:AZURE_VISION_ENDPOINT" }

# If any keys are stored in keyvault provide the keyvault name and secret name
if ($env:AZURE_KEY_VAULT_NAME) { $keyVaultName = "--keyvaultname $env:AZURE_KEY_VAULT_NAME" }
if ($env:AZURE_SEARCH_SECRET_NAME) { $searchSecretNameArg = "--searchsecretname $env:AZURE_SEARCH_SECRET_NAME" }
...
if ($env:USE_LOCAL_HTML_PARSER -eq $true) { $localHtmlParserArg = "--localhtmlparser" }

if ($env:AZURE_TENANT_ID) { $tenantArg = "--tenantid $env:AZURE_TENANT_ID" }

if ($env:USE_FEATURE_INT_VECTORIZATION) { $integratedVectorizationArg = "--useintvectorization $env:USE_FEATURE_INT_VECTORIZATION" }
